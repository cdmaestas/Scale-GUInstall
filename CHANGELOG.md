...
- The same class of danger existed for the web UI, independent of the MCP fix above: none of the documented or generated SSH tunnel commands (README, man page, help.html, the Settings page's tunnel helper) included any SSH keepalive options. An idle NAT or firewall between a workstation and the installer node can silently drop the raw tunnel connection during the long quiet stretches install/deploy/upgrade phases normally have — which the backend sees as an abandoned request and responds to the same way, by killing the still-running command. The web UI's own `fetch()`-based streaming has no client-side timeout of its own (confirmed by reading it), so this was purely a documentation/generated-command gap, not a code bug — every tunnel command now includes `-o ServerAliveInterval=30 -o ServerAliveCountMax=3`.
