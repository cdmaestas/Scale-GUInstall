...
### Added
- MCP parity for the web UI's "Post Configuration" page: `start_profiled`, `start_guiuser`, `start_mmchconfig_tunables`, `start_healthinterval`, `start_nfs_core_dump`, `start_afmgateway`. All six backend endpoints (`postconfig/profiled`, `postconfig/guiuser`, `postconfig/mmchconfig`, `postconfig/healthinterval`, `nfs-core-dump`, `postconfig/afmgateway`) had been wired into the web UI for a while but never exposed via MCP, and none had dry-run/operation-buffer support at all — same class of gap as the pre-fix `config populate`/`callhome`. `dry_run` defaults to `False` on all six (unlike newer endpoints, which default `True`) specifically because the existing web UI calls never send this param — defaulting `True` would have silently turned every existing "run for real" button into a no-op. Also fixed a latent ordering bug in `afmgateway` found while adding this: it validated `nfs_target`/S3 field presence *after* already running `mmcrfileset`, so a request missing those fields left an orphaned fileset behind; validation now happens before any command runs.
...
- `--ephemeral_port_range` added to `_ALLOWED_GPFS_FLAGS` — found live via `install --precheck`'s own WARN recommending `./spectrumscale config gpfs --ephemeral_port_range 60000-61000`, which this backend rejected as an "Unrecognised flag" before the fix. Shared allowlist, so this also unblocks it via `start_cluster_config_apply`'s `gpfs_flags`.
- `POST /api/stream/callhome` (`spectrumscale callhome enable|disable`) + `start_callhome` MCP tool. Found as a real gap live: `install --precheck` FATALs when call home is enabled (the toolkit's default since 5.0.0) but never configured, and there was previously no way to disable it through this backend — the only existing call-home path, `start_cluster_config_apply`'s `callhome` flag, only ever enables it as part of a broader config-apply, and the underlying GET-based `/api/stream/callhome` endpoint it called into had no dry-run or operation-buffer support at all (same class of gap as the pre-fix `config populate`). Rebuilt as its own POST+JSON endpoint with dry-run/operation-buffer support, and correctly checks the command's real exit code rather than reusing the shared `_gen_callhome` helper's yield-from pattern, which doesn't surface `rc` to its caller — `cluster-config-apply`'s existing usage of that helper has a latent bug where it marks the whole operation "success" even if the callhome/perfmon/fileaudit step inside it failed; left alone here since fixing it is out of scope for this endpoint.
...
- `POST /api/stream/gpfs/shutdown` / `POST /api/stream/gpfs/startup` (`mmshutdown -N <nodes>` / `mmstartup -N <nodes>`) + `start_gpfs_shutdown`/`start_gpfs_startup` MCP tools. Discovered as a real prerequisite live against the cluster: `upgrade config offline` refuses a node whose GPFS daemon is still running ("cannot be designated for offline upgrade until the GPFS daemon running on the node is stopped"), and `upgrade run` deliberately never restarts GPFS on an offline-designated node afterward — both ends of that lifecycle needed their own endpoint.
...
- `postconfig/profiled` (`start_profiled`) reworked from a local `cp` on the installer node to SSH per cluster node — found live while trying it: GPFS binaries don't need PATH help from this script on the installer node, which already knows its own binary paths; the actual GPFS cluster nodes are where users log in and run mmXXX commands by hand. Now takes a required `nodes` list and creates `/etc/profile.d/gpfs.sh` on each one over SSH, matching the pattern `list_devices`/`test_connection` already use for reaching remote nodes. **Breaking**: `start_profiled` now requires a `nodes` argument it didn't take before. The web UI's Environment Setup panel gained a "Target Nodes" field (pre-filled from Node Configuration) to match.

### Fixed
- `start.sh`'s Python/pip install suggestions were too generic to be directly actionable: the "no Python found" message now suggests the interpreter and its matching pip package together (`sudo apt install python3.11 python3.11-pip` / `sudo yum install ...`), and the "pip missing" message now names the pip package matching whichever interpreter `$PYTHON` actually resolved to (e.g. `python3.12-pip`) instead of the generic `python3-pip`, which can map to a different version than the one in use.
...
- Hardening pass (security review, silent-failure audit, git hook coverage, doc drift), second pass following the earlier one in 1.2.0:
  ...
  - Added a `pip-audit` job to CI (dependency vulnerability scanning), isolated into its own venv after the first version audited the runner's global environment and false-positived on GitHub's pre-installed tooling — nothing found on the project's actual dependencies.
...
- Backend support for the above, usable independent of MCP: every mutating endpoint now accepts a `dry_run` flag that validates inputs and reports what would run without ever spawning a subprocess (new SSE event type `"dryrun"`), and a new single-slot operation buffer (`GET /api/operation/current`) tracks the one mutating operation allowed to run at a time — including which client (web UI or MCP) started it — so a second attempt from either interface while one is in flight gets a clear "busy" response instead of silently colliding. `stream_process()` also gained a `finally: proc.kill()` safety net so an abandoned SSE connection (e.g. an MCP client that reads one event and disconnects) can no longer leave its child process running or orphaned — a latent gap that predates MCP but became far more likely to hit with a fire-and-forget caller in the picture. Caught and fixed a follow-on bug in that same safety net during testing: `proc.kill()` alone left a zombie process behind (confirmed via `ps`) because nothing reaped it — added the matching `proc.wait()`, mirroring the pattern the pre-existing timeout branch already used.

### Fixed
- A real, long-running mutating operation (e.g. `install`) could get its child process killed mid-run for no reason related to the operation itself: the MCP client's shared `httpx.AsyncClient` used one flat 30-second timeout for every request, including the read side of a fire-and-forget mutating call's background drain — so a toolkit phase that legitimately went quiet for longer than 30s (which `install`/`deploy` routinely do) raised `httpx.ReadTimeout`, abandoning the connection, which `stream_process()`'s abandoned-connection safety net (added for reaping zombies from genuinely-abandoned test connections) then treated as a signal to kill the still-running, still-useful child. Confirmed concretely: retrying the same install with no MCP timeout in place let it run its full toolkit sequence to completion. `ScaleBackendClient.open_stream()` now opens with no read timeout specifically (connect/write/pool stay bounded, so an unreachable backend still fails fast) instead of changing the shared client's default for every request. This is the narrow "interim guard" from `docs/long-running-operations-plan.md`, not the full fix that plan describes — the backend's kill-on-abandon behavior is deliberately left alone, since removing it without also restructuring who keeps draining a child's stdout once its HTTP watcher disconnects would trade a loud failure (killed process) for a silent one (child blocks forever on a full stdout pipe, invisible anywhere). Also: the MCP background drain task now logs instead of silently discarding an exception if the connection is lost for a genuinely different reason.
- The same class of danger existed for the web UI, independent of the MCP fix above: none of the documented or generated SSH tunnel commands included any SSH keepalive options — README, the man page, help.html, the Settings page's tunnel helper, and (caught in a follow-up pass) the tunnel commands `start.sh` and the packaged `scale-guinstall` wrapper both print at startup. An idle NAT or firewall between a workstation and the installer node can silently drop the raw tunnel connection during the long quiet stretches install/deploy/upgrade phases normally have — which the backend sees as an abandoned request and responds to the same way, by killing the still-running command. The web UI's own `fetch()`-based streaming has no client-side timeout of its own (confirmed by reading it), so this was purely a documentation/generated-command gap, not a code bug — every tunnel command now includes `-o ServerAliveInterval=30 -o ServerAliveCountMax=3`. The wrapper's startup message also had the same stale "open Scale-GUInstall.html directly" guidance already fixed elsewhere in `start.sh` — fixed there too.

### Fixed
- Every one of the 8 dry-run-aware mutating code paths (setup, node config's per-node and summary lines, cluster-config-apply's config-gpfs/call-home/perfmon/file-audit steps, NSD add, format-disk, and the generic phase runner) yielded the exact same `"success"`-typed, real-completion-worded event whether or not anything actually ran — because a dry run's `stream_process()` call always returns `rc=0`, and every caller's `if rc == 0:` branch never checked `dry_run` before picking its message. Found by testing `start_setup`'s dry run against a real cluster over the new MCP server and seeing `"[OK] Installation service setup complete."` immediately after a line stating the command was never executed. New `sse_final_status()` helper picks between a real `"success"` event and a `"dryrun"`-typed one (reusing the type `stream_process()` itself already emits) so a consumer that only checks event type — the MCP server, or any future non-web client — can never mistake dry-run validation for confirmation that something executed.
