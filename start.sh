...
echo "    ssh -o ServerAliveInterval=30 -o ServerAliveCountMax=3 -L $PORT:127.0.0.1:$PORT <user>@<scale-node>"
echo "  (the keepalive options above matter for long install/deploy/upgrade"
echo "   phases — without them an idle NAT or firewall can silently drop the"
echo "   tunnel mid-phase, which looks like an abandoned request and gets"
echo "   the still-running command killed, not just the connection)"
